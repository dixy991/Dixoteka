      ...
      <v-card class="text-center mb-0" width="300">
        <v-card-text>
          ...
          <v-btn class="ma-1" color="#55acee" dark default rounded>
            <v-icon>mdi-twitter</v-icon>- 45
          </v-btn>

          <v-btn class="ma-1" color="#3b5998" dark default rounded>
            <v-icon>mdi-facebook</v-icon>- 50
          </v-btn>
        </v-card-text>
      </v-card>
    </v-menu>
...
export default {
  ...
  watch: {
    ...
    showImg(val) {
      if (!val) {
        this.saveImage = this.$store.state.barImage;
        this.$store.commit("SET_BAR_IMAGE", "");
      } else if (this.saveImage) {
        this.$store.commit("SET_BAR_IMAGE", this.saveImage);
        this.saveImage = "";
      } else {
        this.$store.commit("SET_BAR_IMAGE", val);
      }
    },
    image(val) {
      this.$store.commit("SET_BAR_IMAGE", val);
